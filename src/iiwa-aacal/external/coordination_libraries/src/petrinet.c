...
int create_token_type(place_t *p, char *type_name, int n) {
    count_of_token_t *rec = (count_of_token_t *) agbindrec(p, type_name, sizeof(count_of_token_t), 0);
    if (rec) {
        rec->count = n;
        snprintf(rec->token_type, MAX_LENGTH_TOKEN_TYPE_NAME, "%s", type_name);
...
void delete_token_type(place_t *p, char *type_name) {
    count_of_token_t *rec = (count_of_token_t *) aggetrec(p, type_name, 0);
    if (rec) {
        agdelrec(p, type_name);
        char *number_of_token_types = agget(p, "number_of_token_types");
        char number[4];
        snprintf(number, 4, "%d", (number_of_token_types ? atoi(number_of_token_types) : 0) - 1);
        agset(p, "number_of_token_types", number);
    }
    assert ((count_of_token_t *) aggetrec(p, type_name, 0) == NULL);
}

void add_tokens(place_t *p, char *type_name, int n) {
    count_of_token_t *rec = NULL;
    if ( (rec = (count_of_token_t *) aggetrec(p, type_name, 0)) ) {
        rec->count += n;
    } else {
        create_token_type(p, type_name, n);
    }
}

int delete_tokens(place_t *p, char *type_name, int n) {
    count_of_token_t *rec = (count_of_token_t *) aggetrec(p, type_name, 0);
    if (rec) { // the token type exists
        if (n < rec->count) {
            rec->count -= n;
        } else {
            delete_token_type(p, type_name);
...
int get_number_of_tokens(place_t *p, char *type_name) {
    count_of_token_t *rec = (count_of_token_t *) aggetrec(p, type_name, 0);
    if (rec) {
        return rec->count;
    } else {
...
bool check_number_of_tokens(place_t *p, char *type_name, int n) {

    int number_of_tokens = get_number_of_tokens(p, type_name);
