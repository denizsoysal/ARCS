...
void iiwaClient::command()
{
	...
	if (robotState().getClientCommandMode() == POSITION) {
		...
		// printf("Commanding wrench %f, %f, %f, %f, %f, %f \n", 
		// 					cmd_wrench[0], cmd_wrench[1], cmd_wrench[2], 
		// 					cmd_wrench[3], cmd_wrench[4], cmd_wrench[5]);
		LBRClient::command();
		robotCommand().setWrench(cmd_wrench);
	}
}
...
void iiwaClient::getDiscreteState(){
	...
	current_sesion_state = this->robotState().getSessionState();
